...
import { findSlotById, lockSlotForUpdate, markSlotBooked, markSlotBookedIfAvailable } from "../repositories/slot.repository.js";
...
export async function listHostBookings(hostId: number, query: ListHostBookingsQuery) {
    ...
    const to = query.from
        ? DateTime.fromISO(query.from, { zone: "utc" }).endOf("day").toJSDate()
        : undefined;

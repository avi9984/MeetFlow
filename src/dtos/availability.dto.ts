...
export const updateAvailabilityExceptionSchema = availabilityExceptionBaseSchema.partial().superRefine((data, ctx) => {
    if (data.type !== undefined && data.type !== 'BLOCK_FULL_DAY') {
        if (!data.startTime) {
            ctx.addIssue({ path: ['startTime'], code: 'custom', message: 'Start time is required for a non-full day exception' })
        }
        if (!data.endTime) {
